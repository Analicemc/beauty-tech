...
class PessoaList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Listagem de pessoa");
        $this->limit = 20;
        ...
        $id = new TEntry('id');
        $nome = new TDBEntry('nome', 'meusalao', 'Pessoa', 'nome','nome asc' , $criteria_nome );
        ...
        if(TSession::getValue('PessoaList_expand_start_hidden') === false)
        {
            $startHidden = false;
        }
        elseif(TSession::getValue('PessoaList_expand_start_hidden') === true)
        {
            $startHidden = true; 
        }
        $expandButton = $this->form->addExpandButton("Filtros", 'fas:filter ', $startHidden);
        ...
        $column_nome_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            if($value)
        ...
        $column_fone_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {

            if($value){
                $value = TextService::limpar($value);
                $t_formatado = TransformationService::formatPhone($value);
                $icon = "<i class='fab fa-whatsapp'></i>";
                $link = "https://wa.me/55$value?text=";
                return "<a href='$link' target='_blank'> $icon $t_formatado </a>";
            }
        ...
        $action_onShow->setLabel("Ver detalhes");
...
//</generated-DatagridHeaderAction-onClearFilters>
    ...
    public static function onExpandForm($param = null)
    {
        try
        {
            ...
            if(TSession::getValue('PessoaList_expand_start_hidden') === false)
            {
                TSession::setValue('PessoaList_expand_start_hidden', true);
            }
            elseif(TSession::getValue('PessoaList_expand_start_hidden') === true)
            {
                TSession::setValue('PessoaList_expand_start_hidden', false);
            }
            else
            {
                TSession::setValue('PessoaList_expand_start_hidden', !$startHidden);
            }
