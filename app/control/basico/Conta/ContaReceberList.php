...
class ContaReceberList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $filtros_rapidos->addItems(["atrasadas" => "Buscar atrasadas", "abertas" => "Buscar abertas", "quitadas" => "Buscar quitadas"]);
        ...
        $data_vencimento_final->setSize(130);
        ...
        $row2 = $this->form->addFields([new TLabel("Cliente:", null, '14px', null, '100%'), $pessoa_id]);
        $row2->layout = ['col-sm-6'];

        $row3 = $this->form->addFields([new TLabel("Categoria:", null, '14px', null, '100%'), $categoria_id], [new TLabel("Forma de pagamento:", null, '14px', null, '100%'), $forma_pagamento_id]);
        $row3->layout = ['col-sm-6', 'col-sm-6'];

        $row4 = $this->form->addFields([new TLabel("Data de vencimento:", null, '14px', null, '100%'), $dt_vencimento, new TLabel("até", null, '14px', null), $data_vencimento_final], [new TLabel("Filtros rápidos:", null, '14px', null, '100%'), $filtros_rapidos]);
        ...
        if (TSession::getValue('ContaReceberList_expand_start_hidden') === false) {
            $startHidden = false;
        } elseif (TSession::getValue('ContaReceberList_expand_start_hidden') === true) {
            $startHidden = true;
        }
        ...
        $column_parcela = new TDataGridColumn('parcela', "Parcela", 'right');
        ...
        $button_cadastrar_varias_contas = new TButton('button_button_cadastrar_varias_contas');
        $button_cadastrar_varias_contas->setAction(new TAction(['ContaReceberLoteForm', 'onShow']), "Cadastrar várias contas");
        $button_cadastrar_varias_contas->addStyleClass('btn-default');
        $button_cadastrar_varias_contas->setImage('fas:plus #4CAF50');

        $this->datagrid_form->addField($button_cadastrar_varias_contas);

        $button_quitar_contas_selecionadas = new TButton('button_button_quitar_contas_selecionadas');
        $button_quitar_contas_selecionadas->setAction(new TAction(['QuitarContaReceberLoteForm', 'onShow']), "Quitar contas selecionadas");
        $button_quitar_contas_selecionadas->addStyleClass('btn-default');
        $button_quitar_contas_selecionadas->setImage('fas:calendar-check #4CAF50');

        $this->datagrid_form->addField($button_quitar_contas_selecionadas);
        ...
        $head_left_actions->add($button_cadastrar_varias_contas);
        $head_left_actions->add($button_quitar_contas_selecionadas);
    ...
    public function onDelete($param = null)
    {
        if (isset($param['delete']) && $param['delete'] == 1) {
            try {
                ...
                // instantiates object
                $object = new Conta($key, FALSE); //</blockLine>
    ...
    //</generated-DatagridAction-onDelete>
    //<generated-DisplayCondition-onExibirQuitarConta>
    ...
    public static function onExpandForm($param = null)
    {
        try {
            ...
            if (TSession::getValue('ContaReceberList_expand_start_hidden') === false) {
                TSession::setValue('ContaReceberList_expand_start_hidden', true);
            } elseif (TSession::getValue('ContaReceberList_expand_start_hidden') === true) {
                TSession::setValue('ContaReceberList_expand_start_hidden', false);
            } else {
                TSession::setValue('ContaReceberList_expand_start_hidden', !$startHidden);
            }
